#!/usr/bin/env python3
# 🜏 PILL RED: SCIENTIFIC VISUALIZATION FOR EXP-PHASE11-HODGE-PARITY-CRUCIBLE-001 🜏
import os
import sys
import json
...
dataset_file = os.path.join(parent, "evidence", "BENCHMARK_RECORDS", "EXP_PHASE11_HODGE_DATASET.json")
...
conf_h = [r["mode_h_conflicts"] for r in instances]
triangles = [r["triangles"] for r in instances]

# Create 4-Panel Plot
plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')
fig, axs = plt.subplots(2, 2, figsize=(15, 10), dpi=300)
fig.suptitle("PILL RED Phase XI: Hodge Laplacian & Parity Obstruction Crucible\n(EXP-PHASE11-HODGE-PARITY-CRUCIBLE-001 | Tseitin Expander Parity Formulas, 15 Instances)", fontsize=14, fontweight='bold')

x = np.arange(len(inst_ids))
width = 0.25

# Panel A: 3-Way Conflict Comparison
...
axs[0, 0].bar(x + width, conf_h, width, label='Mode H (2D Hodge $\\Delta_1$)', color='#2ca02c', alpha=0.85, edgecolor='black')
...
labels = ["Mode E ($L_0$)", "Mode H (Hodge $\\Delta_1$)"]
reductions = [summary["reduction_l0_pct"], summary["reduction_hodge_pct"]]
...
axs[0, 1].set_ylim(0, max(15, max(reductions) * 1.3))
for i, v in enumerate(reductions):
    axs[0, 1].text(i, v + 0.5, f"{v:+.1f}%", ha='center', fontweight='bold', fontsize=11)
axs[0, 1].grid(True, alpha=0.3)

# Panel C: Triangles (2-Simplices) vs Expander Girth
axs[1, 0].bar(x, triangles, color='#9467bd', alpha=0.85, edgecolor='black', width=0.5)
axs[1, 0].set_title("Panel C: Number of 2-Simplices (Triangles) per Instance", fontweight='bold')
axs[1, 0].set_xticks(x)
axs[1, 0].set_xticklabels([f"I{i}" for i in inst_ids], fontsize=8)
axs[1, 0].set_ylabel("Triangle Count (Local 2-Cycles)")
for i, v in enumerate(triangles):
    axs[1, 0].text(i, v + 0.1, f"{v}", ha='center', fontweight='bold', fontsize=8)
...
axs[1, 1].set_title("Panel D: Ground-Truth SAT/UNSAT Agreement Rate", fontweight='bold')
...
out_png = os.path.join(release_dir, "phase11_hodge_parity.png")
