...
```
       ...
       ┌───────────────────────────┼───────────────────────────┐
       ▼                           ▼                           ▼
   LEAN 4 PROOFS             ROCQ PROVER 9.0.1            KANI MODEL
  (Lake Verified)             (coqc Verified)         (Symbolic Proofs / CI)
       │                           │                           │
  `lake build`                `coqc` 9.0.1            `cargo test` & Kani
...
| Theorem | Mathematical Proposition | Protocol Spec (`PILLRED-SPEC-1.0`) | Rust Core (`pill_red_core::protocol`) | Kani Harness (Direct Production Calls) | Rocq 9.0.1 Theorem | Lean 4 Theorem | Classification |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| **$P_2$** | **Temporal Precedence** | $t_{\text{commit}} < t_{\text{event}} \le t_{\text{resolve}}$ | `verify_single` temporal check | `check_temporal_precedence_invariant` | `temporal_soundness` | `temporal_precedence_soundness_thm` | **PROVED** |
...
| **$P_5$** | **Passport Seal Binding** | Section alteration invalidates seal | `verify_passport` hash check | `check_passport_section_tamper_detection` | `passport_soundness` | `passport_tamper_evidence_thm` | **PROVED** |
...
3. **Kani Symbolic Proof CI Workflow** ([`.github/workflows/kani.yml`](file:///C:/PILL%20RED/.github/workflows/kani.yml)):
   - Runs `model-checking/kani-github-action@v1` with `--all-targets`.
